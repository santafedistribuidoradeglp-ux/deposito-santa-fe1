...
- 2026-06 (fork atual):
  ...
  - 2026-08 — Revisão de qualidade: `create_order` foi dividido em validação, precificação e montagem do documento; regras de cupom foram agrupadas em verificações declarativas; contexto de autenticação passou a memorizar callbacks e o valor compartilhado. Verificado: alerta sobre variável `o` era falso positivo (generator expressions locais), chaves de pedidos já usam `order.id`, e dependências atuais de hooks usam apenas constantes/imports, setters ou refs estáveis. Componentes grandes ficam como melhoria estrutural P2 para evitar risco desnecessário em fluxo de pedido já validado.
  - 2026-08 — Verificação da revisão: compilação Python e build React passaram; `backend/tests/backend_test.py` passou; endpoint de pedido validou carrinho vazio com 400 e o catálogo continuou com 4 produtos. Pedido temporário de teste removido, base voltou a 0 pedidos.
  - 2026-08 — Indicação “nem comprou e já ganhou”: cadastro agora libera código/link próprio imediatamente; contas antigas são liberadas ao consultar o link; crédito de indicação continua condicionado a pedido P13 de outra pessoa. Card de indicação não bloqueia mais o compartilhamento e o botão flutuante agora abre o compartilhamento do link (ícone Share2) mantendo o verde original. Testado pelo agente: backend 4/4 e frontend completo, sem APIs simuladas.
